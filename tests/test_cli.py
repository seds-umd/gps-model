...
spec = importlib.util.spec_from_file_location('gps_cli', Path(__file__).parents[1] / 'main.py')
...
class CaptureCliTests(unittest.TestCase):
    ...
    def test_invalid_capture_and_options_fail_with_useful_messages(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            cases = [(['missing.iq'], 'does not exist'),
                     (['empty.iq'], 'empty'),
                     (['odd.iq'], 'complete I/Q'),
            ...
            for args, message in cases:
                with self.subTest(args=args), patch.object(cli.gps_receiver, 'GpsReceiver') as receiver:
                    result = runner.invoke(cli.main, [str(Path(directory, args[0])), *args[1:]])
    ...
    def test_signed_iq_and_partial_final_chunk(self):
        ...
        for width in [8, 16, 32]:
            with self.subTest(width=width), tempfile.TemporaryDirectory() as directory:
                values = np.array([-7, 3, 2, -5, 0, 1, 6, 4, -2, -3], dtype=f'<i{width // 8}')
                capture = Path(directory, 'capture.iq')
                values.tofile(capture)
                with patch.object(cli.gps_receiver, 'GpsReceiver') as receiver:
                    result = runner.invoke(cli.main, [str(capture), '--width', str(width), '--fs', '2'])
                    self.assertEqual(result.exit_code, 0, repr(result.exception))
                    chunks = [c.args[0] for c in receiver.return_value.process.call_args_list]
                    self.assertEqual([len(c) for c in chunks], [2, 2, 1])
                    np.testing.assert_array_equal(np.concatenate(chunks), values[::2] + 1j * values[1::2])
